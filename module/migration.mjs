...
export const migrateWorld = async function() {
  ...
  game.settings.set("dnd5e-2014", "systemMigrationVersion", game.system.version);
...
export function migrateItemData(item, migrationData, flags={}) {
  ...
  // Migrate properties
  const migratedProperties = foundry.utils.getProperty(item, "flags.dnd5e-2014.migratedProperties");
  if ( migratedProperties?.length ) {
    ...
    updateData["flags.dnd5e-2014.-=migratedProperties"] = null;
  }

  if ( foundry.utils.getProperty(item, "flags.dnd5e-2014.persistSourceMigration") ) {
    flags.persistSourceMigration = true;
    updateData["flags.dnd5e-2014.-=persistSourceMigration"] = null;
  }
...
export const getMigrationData = async function() {
  const data = {};
  try {
    const icons = await fetch("systems/dnd5e-2014/json/icon-migration.json");
    const spellIcons = await fetch("systems/dnd5e-2014/json/spell-icon-migration.json");
...
function _migrateTokenImage(actorData, updateData) {
  ...
  for ( const path of ["texture.src", "prototypeToken.texture.src"] ) {
    ...
    if ( oldSystemPNG.test(v) ) {
      ...
      updateData[path] = `systems/dnd5e-2014/tokens/${type}/${fileName}.webp`;
    }
  }
  return updateData;
}
...
export async function purgeFlags(pack) {
  const cleanFlags = flags => {
    const flags5e = flags.dnd5e-2014 || null;
