...
export default class Actor5e extends SystemDocumentMixin(Actor) {
   ...
   */
  get isPolymorphed() {
    return this.getFlag("dnd5e-2014", "isPolymorphed") || false;
  }
  ...
  get concentration() {
    ...
    for ( const effect of this.effects ) {
      ...
      const data = effect.getFlag("dnd5e-2014", "itemData");
  ...
  *allApplicableEffects() {
    for ( const effect of super.allApplicableEffects() ) {
      if ( (effect.getFlag("dnd5e-2014", "type") !== "enchantment") && !effect.getFlag("dnd5e-2014", "rider") ) yield effect;
    }
  }
  ...
  /** @inheritDoc */
  prepareDerivedData() {
    const origin = this.getFlag("dnd5e-2014", "summon.origin");
   ...
   *                                             `this.flags.dnd5e-2014.originalActor` are used.
  ...
  async endConcentration(target) {
    ...
    if ( foundry.utils.getType(target) === "string" ) effect = effects.find(e => e.id === target);
      ...
      effect = effects.find(e => {
        const data = e.getFlag("dnd5e-2014", "itemData") ?? {};
  ...
  async challengeConcentration({ dc=10, ability=null }={}) {
    ...
    return ChatMessage.implementation.create({
      content: await renderTemplate("systems/dnd5e-2014/templates/chat/request-card.hbs", {
  ...
  _isRemarkableAthlete(ability) {
    return this.getFlag("dnd5e-2014", "remarkableAthlete")
  ...
  async rollSkill(skillId, options={}) {
    ...
    const reliableTalent = (skl.value >= 1 && this.getFlag("dnd5e-2014", "reliableTalent"));
    ...
    const rollData = foundry.utils.mergeObject({
      ...
      flavor,
      chooseModifier: true,
      halflingLucky: this.getFlag("dnd5e-2014", "halflingLucky"),
      reliableTalent,
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.dnd5e-2014.roll": {type: "skill", skillId }
      }
    }, options);
  ...
  async rollToolCheck(toolId, options={}) {
    ...
    const reliableTalent = (prof?.multiplier >= 1 && this.getFlag("dnd5e-2014", "reliableTalent"));
    ...
    const rollData = foundry.utils.mergeObject({
      ...
      messageData: {
        ...
        "flags.dnd5e-2014.roll": {type: "tool", toolId}
      }
    }, options);
    rollData.parts = parts.concat(options.parts ?? []);
  ...
  async rollAbilityTest(abilityId, options={}) {
    ...
    const rollData = foundry.utils.mergeObject({
      ...
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.dnd5e-2014.roll": {type: "ability", abilityId }
      }
    }, options);
    rollData.parts = parts.concat(options.parts ?? []);
  ...
  async rollAbilitySave(abilityId, options={}) {
    ...
    const rollData = foundry.utils.mergeObject({
      ...
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.dnd5e-2014.roll": {type: "save", abilityId }
      }
    }, options);
    rollData.parts = parts.concat(options.parts ?? []);
  ...
  async rollDeathSave(options={}) {
    ...
    if ( this.getFlag("dnd5e-2014", "diamondSoul") ) {
      parts.push("@prof");
    ...
    const rollData = foundry.utils.mergeObject({
      ...
      messageData: {
        speaker: speaker,
        "flags.dnd5e-2014.roll": {type: "death"}
      }
    }, options);
    rollData.parts = parts.concat(options.parts ?? []);
  ...
  getInitiativeRoll(options={}) {
    ...
    const tiebreaker = game.settings.get("dnd5e-2014", "initiativeDexTiebreaker");
  ...
  async rollHitDie(denomination, options={}) {
    ...
    const rollConfig = foundry.utils.mergeObject({
      ...
      messageData: {
        ...
        "flags.dnd5e-2014.roll": {type: "hitDie"}
      }
    }, options);
  ...
  async rollClassHitPoints(item, { chatMessage=true }={}) {
    ...
    const messageData = {
      ...
      flavor,
      speaker: ChatMessage.implementation.getSpeaker({ actor: this }),
      "flags.dnd5e-2014.roll": { type: "hitPoints" }
    };
  ...
  async shortRest(config={}) {
    ...
    config = foundry.utils.mergeObject({
      ...
      duration: CONFIG.DND5E.restTypes.short.duration[game.settings.get("dnd5e-2014", "restVariant")]
    }, config);
  ...
  async longRest(config={}) {
    ...
    config = foundry.utils.mergeObject({
      ...
      duration: CONFIG.DND5E.restTypes.long.duration[game.settings.get("dnd5e-2014", "restVariant")]
    }, config);
  ...
  async _displayRestResultMessage(result, longRest=false) {
    ...
    let restFlavor;
    switch (game.settings.get("dnd5e-2014", "restVariant")) {
      case "normal":
    ...
    let chatData = {
      ...
      "flags.dnd5e-2014.rest": { type: longRest ? "long" : "short" }
    };
  ...
  async _getRestItemUsesRecovery({recoverShortRestUses=true, recoverLongRestUses=true,
    ...
    for ( let item of this.items ) {
      ...
      if ( recoverDailyUses && uses.recovery && CONFIG.DND5E.limitedUsePeriods[uses.per]?.formula ) {
        ...
        if ( recoverLongRestUses && (game.settings.get("dnd5e-2014", "restVariant") === "gritty") ) {
  ...
  async transformInto(target, { keepPhysical=false, keepMental=false, keepSaves=false, keepSkills=false,
    ...
    const allowed = game.settings.get("dnd5e-2014", "allowPolymorphing");
    ...
    if ( game.release.generation >= 12 ) d.prototypeToken.ring = source.prototypeToken.ring;
      ...
      foundry.utils.setProperty(d.prototypeToken, "flags.dnd5e-2014.tokenRing", foundry.utils.mergeObject(
        foundry.utils.getProperty(d.prototypeToken, "flags.dnd5e-2014.tokenRing") ?? {},
        foundry.utils.getProperty(source.prototypeToken, "flags.dnd5e-2014.tokenRing") ?? {},
        { inplace: false }
      ));
    }
    ...
    if ( this.isToken ) {
      ...
      foundry.utils.setProperty(tokenData, "flags.dnd5e-2014.previousActorData", previousActorData);
    ...
    const updates = tokens.map(t => {
      ...
      const dOriginalActor = foundry.utils.getProperty(d, "flags.dnd5e-2014.originalActor");
      foundry.utils.setProperty(newTokenData, "flags.dnd5e-2014.originalActor", dOriginalActor);
      foundry.utils.setProperty(newTokenData, "flags.dnd5e-2014.isPolymorphed", true);
      return newTokenData;
    });
  ...
  async revertOriginalForm({renderSheet=true}={}) {
    ...
    const original = game.actors.get(this.getFlag("dnd5e-2014", "originalActor"));
    ...
    if ( this.isToken ) {
      ...
      if ( !baseActor ) {
        ui.notifications.warn(game.i18n.format("DND5E.PolymorphRevertNoOriginalActorWarn", {
          reference: this.getFlag("dnd5e-2014", "originalActor")
        }));
        return;
      }
      const prototypeTokenData = (await baseActor.getTokenDocument()).toObject();
      const actorData = this.token.getFlag("dnd5e-2014", "previousActorData");
      ...
      if ( game.release.generation >= 12 ) tokenUpdate.ring = prototypeTokenData.ring;
        ...
        foundry.utils.setProperty(tokenUpdate, "flags.dnd5e-2014.tokenRing", foundry.utils.mergeObject(
          foundry.utils.getProperty(tokenUpdate, "flags.dnd5e-2014.tokenRing") ?? {},
          foundry.utils.getProperty(prototypeTokenData, "flags.dnd5e-2014.tokenRing") ?? {},
          { inplace: false }
        ));
      }
    ...
    if ( canvas.ready ) {
      ...
      const tokenUpdates = tokens.map(t => {
        ...
        delete update.x;
        delete update.y;
        if ( (game.release.generation < 12) && !foundry.utils.getProperty(tokenData, "flags.dnd5e-2014.tokenRing") ) {
          foundry.utils.setProperty(update, "flags.dnd5e-2014.tokenRing", {});
        }
        return update;
      });
  ...
  static addDirectoryContextOptions(html, entryOptions) {
    entryOptions.push({
      ...
      callback: li => {
        game.settings.set("dnd5e-2014", "primaryParty", { actor: game.actors.get(li[0].dataset.documentId) });
      },
      condition: li => {
        const actor = game.actors.get(li[0].dataset.documentId);
        const primary = game.settings.get("dnd5e-2014", "primaryParty")?.actor;
      ...
      callback: li => {
        game.settings.set("dnd5e-2014", "primaryParty", { actor: null });
      },
      condition: li => {
        const actor = game.actors.get(li[0].dataset.documentId);
        const primary = game.settings.get("dnd5e-2014", "primaryParty")?.actor;
  ...
  async _onUpdate(data, options, userId) {
    ...
    if ( hp && !options.isRest && !options.isAdvancement ) {
      ...
      if ( Number.isInteger(changes.total) && (changes.total !== 0) ) {
        ...
        if ( !game.settings.get("dnd5e-2014", "disableConcentration") && (userId === game.userId) && (changes.total < 0) ) {
  ...
  async _onUpdateExhaustion(data, options) {
    ...
    if ( level < 1 ) return effect?.delete();
      ...
      return effect.update({ "flags.dnd5e-2014.exhaustionLevel": level }, { dnd5e: { originalExhaustion } });
    } else {
      effect = await ActiveEffect.implementation.fromStatusEffect("exhaustion", { parent: this });
      effect.updateSource({ "flags.dnd5e-2014.exhaustionLevel": level });
  ...
  updateEncumbrance(options) {
    ...
    if ( !encumbrance || (game.settings.get("dnd5e-2014", "encumbrance") === "none") ) return;
    const statuses = [];
    const variant = game.settings.get("dnd5e-2014", "encumbrance") === "variant";
