...
export default class InventoryElement extends HTMLElement {
  ...
  _getContextOptions(item, element) {
    ...
    const options = [
      ...
      },
      {
        ...
        icon: '<dnd5e-icon src="systems/dnd5e-2014/icons/svg/break-concentration.svg"></dnd5e-icon>',
  ...
  async _onExpand(target, item) {
    ...
    if ( this._app._expanded.has(item.id) ) {
      ...
      const summary = $(await renderTemplate("systems/dnd5e-2014/templates/items/parts/item-summary.hbs", chatData));
