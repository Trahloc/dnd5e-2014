  ...
  return class ActorSheetV2 extends Base {
    ...
    async _renderOuter() {
      ...
      nav.append(...this.constructor.TABS.map(({ tab, label, icon, svg }) => {
        ...
        else if ( svg ) item.innerHTML = `<dnd5e-icon src="systems/dnd5e-2014/icons/svg/${svg}.svg"></dnd5e-icon>`;
        return item;
      }));
    ...
    async _onToggleDescription(event) {
      ...
      if ( expanded ) {
        ...
        summary.innerHTML = await renderTemplate("systems/dnd5e-2014/templates/items/parts/item-summary.hbs", context);
